...
let WhiBoundingBox = require('./WhiBoundingBox.js');
let std_msgs = _finder('std_msgs');

//-----------------------------------------------------------

class WhiBoundingBoxes {
  constructor(initObj={}) {
    if (initObj === null) {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.bounding_boxes.forEach((val) => {
      bufferOffset = WhiBoundingBox.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new WhiBoundingBoxes(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.bounding_boxes[i] = WhiBoundingBox.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.bounding_boxes.forEach((val) => {
      length += WhiBoundingBox.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new WhiBoundingBoxes(null);
    ...
    if (msg.bounding_boxes !== undefined) {
      ...
      for (let i = 0; i < resolved.bounding_boxes.length; ++i) {
        resolved.bounding_boxes[i] = WhiBoundingBox.Resolve(msg.bounding_boxes[i]);
      }
    }
    else {
...
module.exports = WhiBoundingBoxes;
